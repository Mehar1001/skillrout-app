...
// UPDATED: Customer interface to make phone optional
interface Customer {
  id: string;
  name: string;
  phone?: string;
  ...
  useEffect(() => {
    if (searchQuery === '') {
      ...
      const filteredData = customers.filter(customer =>
        ...
        (!customer.phone && customer.id.includes(searchQuery))
      );
  ...
  const renderCustomerCard = ({ item }: { item: Customer }) => (
    <View style={styles.card}>
      <View style={styles.infoRow}>
        <Ionicons name="person-circle-outline" size={24} color="#FFD700" style={styles.icon} />
        <Text style={styles.name}>{item.name}</Text>
      </View>
      {/* UPDATED: Show both phone and ID if available */}
      {item.phone && (
        <View style={styles.infoRow}>
      ...
      <View style={styles.infoRow}>
        <Ionicons name="keypad-outline" size={22} color="#FFD700" style={styles.icon} />
        <Text style={styles.phone}>ID: {item.id}</Text>
      </View>
    ...
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        ...
        <TouchableOpacity onPress={() => router.back()}>
          <Ionicons name="arrow-back-circle-outline" size={32} color="#FFD700" />
        </TouchableOpacity>
      </View>
