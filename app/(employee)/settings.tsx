...
export default function EmployeeSettingsScreen() {
  ...
  const { user, employeeName, email, assignedStoreIds, businessName } = useAuth();
  ...
  const handleForgotPassword = async () => {
    ...
    setLoading(true);
    try {
      await sendSkillroutPasswordReset(auth, user.email, 'employee');
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Card style={styles.card}>
        ...
        <Text style={styles.label}>Assigned Stores</Text>
        <Text style={styles.value}>{assignedStoreIds.length} store(s)</Text>
      </Card>
